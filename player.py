...
class Player():
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        if mode == 'helicopter':
            if len(box_lists) > 0:
                parameters = np.array([[velocity/11],[box_lists[0].gap_mid/420], [agent_position[1]/720], [box_lists[0].x/1280 - agent_position[0]/1280]])
                output = self.nn.forward(parameters)
                if output >= 0.5:
                    direction = 1
                else:
                    direction = -1
        ...
        if mode == 'gravity':
            if len(box_lists) > 0:
                parameters = np.array([[velocity/14],[box_lists[0].gap_mid/360], [agent_position[1]/720], [box_lists[0].x/1280 - agent_position[0]/1280]])
                output = self.nn.forward(parameters)
                if output >= 0.5:
                    direction = 1
                else:
